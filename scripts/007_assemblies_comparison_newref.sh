...
OUT_DIR=/data/users/amaalouf/transcriptome_assembly/assemblies/assemblies_comparison/newref
CONTAINER_SIF=/containers/apptainer/mummer4_gnuplot.sif
REF=/data/users/amaalouf/transcriptome_assembly/assemblies/assemblies_comparison/newref/GCA_946409825.1_Tanz-1.10024.PacbioHiFiAssembly_genomic.fna

# create directory if not available
mkdir -p $OUT_DIR $OUT_DIR/hifiasm $OUT_DIR/hifiasm2 $OUT_DIR/flye2
...
# compare hifiasm assemblies against a new reference
apptainer exec\
 --bind $WORK_DIR\
  $CONTAINER_SIF\
  nucmer \
...
# compare hifiasm assembly against itself
apptainer exec\
 --bind $WORK_DIR\
  $CONTAINER_SIF\
  nucmer \
  ...
  --mincluster 1000\
  --breaklen 1000\
  --delta $OUT_DIR/hifiasm2/comparing_assemblies.delta\
  $ASSEMBLY_HIFIASM\
  $ASSEMBLY_HIFIASM

# compare flye assembly against itself
apptainer exec\
 --bind $WORK_DIR\
  $CONTAINER_SIF\
  nucmer \
  ...
  --mincluster 1000\
  --breaklen 1000\
  --delta $OUT_DIR/flye2/comparing_assemblies.delta\
  $ASSEMBLY_FLYE\
  $ASSEMBLY_FLYE
...
apptainer exec\
 --bind $WORK_DIR\
  ...
  -t png\
  --large \
  -p $OUT_DIR/hifiasm2/mummerplot_output\
  $OUT_DIR/hifiasm2/comparing_assemblies.delta

  apptainer exec\
 --bind $WORK_DIR\
  $CONTAINER_SIF\
  mummerplot -R $ASSEMBLY_FLYE\
  -Q $ASSEMBLY_FLYE \
  --fat \
  --layout \
  --filter \
  ...
  -t png\
  --large \
  -p $OUT_DIR/flye2/mummerplot_output\
  $OUT_DIR/flye2/comparing_assemblies.delta